...
function experimentInit() {
  ...
  end_instr_resp = new core.Keyboard({
    psychoJS: psychoJS,
  ...
  brth_responses = new core.Keyboard({
    psychoJS: psychoJS,
    clock: new util.Clock(),
    waitForStart: true,
  });
...
function InstructionsRoutineBegin(trials) {
  return function () {
    ...
    if (instr_text.text.length > 750) {
      //if there are more than 750 characters, the height is reduced
  ...
  return function () {
    ...
    if (t >= 0.0 && end_instr_resp.status === PsychoJS.Status.NOT_STARTED) {
      ...
      psychoJS.window.callOnFlip(function () {
        end_instr_resp.clock.reset();
      }); // t=0 on next screen flip
      psychoJS.window.callOnFlip(function () {
        end_instr_resp.start();
      }); // start on screen flip
      psychoJS.window.callOnFlip(function () {
        end_instr_resp.clearEvents();
      });
    }

    if (end_instr_resp.status === PsychoJS.Status.STARTED) {
      let theseKeys = end_instr_resp.getKeys({
        keyList: [must_press],
        waitRelease: false,
      });
      ...
      if (_end_instr_resp_allKeys.length > 0) {
        end_instr_resp.keys =
          _end_instr_resp_allKeys[_end_instr_resp_allKeys.length - 1].name; // just the last key pressed
        end_instr_resp.rt =
          _end_instr_resp_allKeys[_end_instr_resp_allKeys.length - 1].rt;
...
function BreathCountingRoutineEachFrame(trials) {
  return function () {
    ...
    if (t >= 0.0 && brth_responses.status === PsychoJS.Status.NOT_STARTED) {
      ...
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function () {
        brth_responses.clock.reset();
      }); // t=0 on next screen flip
      psychoJS.window.callOnFlip(function () {
        brth_responses.start();
      }); // start on screen flip
      psychoJS.window.callOnFlip(function () {
        brth_responses.clearEvents();
      });
    }

    frameRemains = 0.0 + 1200 - psychoJS.window.monitorFramePeriod * 0.75; // most of one frame period left
    if (
      brth_responses.status === PsychoJS.Status.STARTED &&
      t >= frameRemains
    ) {
    ...
    if (brth_responses.status === PsychoJS.Status.STARTED) {
      let theseKeys = brth_responses.getKeys({
    ...
    // *warningText* updates
    if (
      (warningText.status === PsychoJS.Status.NOT_STARTED ||
        warningText.status === PsychoJS.Status.STOPPED) &&
      ((brth_responses.rt.length > 0 &&
        t >= warnTime + brth_responses.rt[brth_responses.rt.length - 1]) ||
        (brth_responses.rt.length === 0 && t >= warnTime))
    ) {
    ...
    if (
      warningText.status === PsychoJS.Status.STARTED &&
      brth_responses.rt.length > 0 &&
      t < warnTime + brth_responses.rt[brth_responses.rt.length - 1]
    ) {
    ...
    if (t >= 0.0 && quit_resp.status === PsychoJS.Status.NOT_STARTED) {
      ...
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function () {
        quit_resp.clock.reset();
      }); // t=0 on next screen flip
      psychoJS.window.callOnFlip(function () {
        quit_resp.start();
      }); // start on screen flip
      psychoJS.window.callOnFlip(function () {
        quit_resp.clearEvents();
      });
    }
    ...
    // check if the Routine should terminate
    if (!continueRoutine || t >= frameRemains) {
      // a component has requested a forced-end of Routine (or time has exceeded)
      return Scheduler.Event.NEXT;
    }
...
function BreathCountingRoutineEnd(trials) {
  return function () {
    ...
    if (typeof brth_responses.keys !== "undefined") {
      // we had a response
    ...
    if (typeof quit_resp.keys !== "undefined") {
      // we had a response
...
function typer(textObject) {
  ...
  if (addText === "return") {
    ...
    var shiftKeyList = [
      ...
      "=",
      "-",
    ]; //non-alphabet characters accessed through the shift key (when modify is true)
...
function importSpreadsheetText(data) {
  ...
  for (let i = 1; i < allRows.length; i++) {
    ...
    if (strtok[key_index] === "instr_demo") {
      let params = {
        instr: strtok[text_index],
        must_press: strtok[must_press_index],
      };
