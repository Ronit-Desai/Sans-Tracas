...
const { round } = util;
const ppc = 37.79; //estimate initial pixel per cm ratio
const creditCardWidthCm = 8.56; // actual credit card dimensions in centimeters
const creditCardHeightCm = 5.398;
...
function experimentInit() {
  ...
  modify_size = new core.Keyboard({
    psychoJS: psychoJS,
    clock: new util.Clock(),
    waitForStart: true,
  });

  force_end_space = new core.Keyboard({
    psychoJS: psychoJS,
    clock: new util.Clock(),
    waitForStart: true,
  });
  ...
  end_instr_movingdot = new core.Keyboard({
    psychoJS: psychoJS,
    clock: new util.Clock(),
    waitForStart: true,
  });
  ...
  resp_enteredBlindSpot = new core.Keyboard({
    psychoJS: psychoJS,
    clock: new util.Clock(),
    waitForStart: true,
  });
...
function CreditCardResizeRoutineEachFrame(trials) {
  return function () {
    ...
    if (t >= 0.0 && modify_size.status === PsychoJS.Status.NOT_STARTED) {
      ...
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function () {
        modify_size.clock.reset();
      }); // t=0 on next screen flip
      psychoJS.window.callOnFlip(function () {
        modify_size.start();
      }); // start on screen flip
      psychoJS.window.callOnFlip(function () {
        modify_size.clearEvents();
      });
    }

    if (modify_size.status === PsychoJS.Status.STARTED) {
      let theseKeys = modify_size.getKeys({
      ...
      if (theseKeys.length !== 0) {
        if (key.name === "up") {
          //increase height if up key is pressed
          creditCard.setSize([w, h + 10]);
        } else if (key.name === "down") {
          //decrease height if down key is pressed
          creditCard.setSize([w, h - 10]);
        } else if (key.name === "left") {
          //increase width if left key is pressed
          creditCard.setSize([w + 15.85, h]);
        } else if (key.name === "right") {
          //decrease height if right key is pressed
          creditCard.setSize([w - 15.85, h]);
        }
      }
      ...
      if (_modify_size_allKeys.length > 0) {
        modify_size.keys =
          _modify_size_allKeys[_modify_size_allKeys.length - 1].name; // just the last key pressed
        modify_size.rt =
          _modify_size_allKeys[_modify_size_allKeys.length - 1].rt;
      }
    }
    ...
    if (t >= 0.0 && force_end_space.status === PsychoJS.Status.NOT_STARTED) {
      ...
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function () {
        force_end_space.clock.reset();
      }); // t=0 on next screen flip
      psychoJS.window.callOnFlip(function () {
        force_end_space.start();
      }); // start on screen flip
      psychoJS.window.callOnFlip(function () {
        force_end_space.clearEvents();
      });
    }

    if (force_end_space.status === PsychoJS.Status.STARTED) {
      let theseKeys = force_end_space.getKeys({
        keyList: ["space"],
        waitRelease: false,
      });
      _force_end_space_allKeys = _force_end_space_allKeys.concat(theseKeys);
      if (_force_end_space_allKeys.length > 0) {
        force_end_space.keys =
          _force_end_space_allKeys[_force_end_space_allKeys.length - 1].name; // just the last key pressed
        force_end_space.rt =
          _force_end_space_allKeys[_force_end_space_allKeys.length - 1].rt;
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function Instructions_BlindSpotRoutineEachFrame(trials) {
  return function () {
    ...
    if (
      t >= 0.0 &&
      end_instr_movingdot.status === PsychoJS.Status.NOT_STARTED
    ) {
      // keep track of start time/frame for later
      ...
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function () {
        end_instr_movingdot.clock.reset();
      }); // t=0 on next screen flip
      psychoJS.window.callOnFlip(function () {
        end_instr_movingdot.start();
      }); // start on screen flip
      psychoJS.window.callOnFlip(function () {
        end_instr_movingdot.clearEvents();
      });
    }

    if (end_instr_movingdot.status === PsychoJS.Status.STARTED) {
      let theseKeys = end_instr_movingdot.getKeys({
        keyList: ["space"],
        waitRelease: false,
      });
      _end_instr_movingdot_allKeys =
        _end_instr_movingdot_allKeys.concat(theseKeys);
      if (_end_instr_movingdot_allKeys.length > 0) {
        end_instr_movingdot.keys =
          _end_instr_movingdot_allKeys[
            _end_instr_movingdot_allKeys.length - 1
          ].name; // just the last key pressed
        end_instr_movingdot.rt =
          _end_instr_movingdot_allKeys[
            _end_instr_movingdot_allKeys.length - 1
          ].rt;
        // a response ends the routine
...
function movingDotTrialRoutineBegin(trials) {
  return function () {
    ...
    var distanceToLeftMargin_cm =
      (distanceToLeftMargin_pix * CC_cm) / creditCard.size[0];
...
function movingDotTrialRoutineEachFrame(trials) {
  return function () {
    ...
    var distanceToLeftMargin_cm =
      (distanceToLeftMargin_pix * creditCardWidthCm) / creditCard.size[0];
    ...
    if (
      t >= 0.0 &&
      resp_enteredBlindSpot.status === PsychoJS.Status.NOT_STARTED
    ) {
      // keep track of start time/frame for later
      ...
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function () {
        resp_enteredBlindSpot.clock.reset();
      }); // t=0 on next screen flip
      psychoJS.window.callOnFlip(function () {
        resp_enteredBlindSpot.start();
      }); // start on screen flip
      psychoJS.window.callOnFlip(function () {
        resp_enteredBlindSpot.clearEvents();
      });
    }

    frameRemains = 0.0 + stop_time - psychoJS.window.monitorFramePeriod * 0.75; // most of one frame period left
    if (
      resp_enteredBlindSpot.status === PsychoJS.Status.STARTED &&
      t >= frameRemains
    ) {
    ...
    if (resp_enteredBlindSpot.status === PsychoJS.Status.STARTED) {
      let theseKeys = resp_enteredBlindSpot.getKeys({
        keyList: ["space"],
        waitRelease: false,
      });
      _resp_enteredBlindSpot_allKeys =
        _resp_enteredBlindSpot_allKeys.concat(theseKeys);
      if (_resp_enteredBlindSpot_allKeys.length > 0) {
        resp_enteredBlindSpot.keys =
          _resp_enteredBlindSpot_allKeys[
            _resp_enteredBlindSpot_allKeys.length - 1
          ].name; // just the last key pressed
        resp_enteredBlindSpot.rt =
          _resp_enteredBlindSpot_allKeys[
            _resp_enteredBlindSpot_allKeys.length - 1
          ].rt;
        // a response ends the routine
    ...
    if (movingDot.status === PsychoJS.Status.STARTED) {
...
function movingDotTrialRoutineEnd(trials) {
  return function () {
    ...
    if (typeof resp_enteredBlindSpot.keys !== "undefined") {
      // we had a response
...
function ExperimentUpLoopBegin(ExperimentUpLoopScheduler) {
  ...
  for (const thisExperimentUp of ExperimentUp) {
    ...
    ExperimentUpLoopScheduler.add(
      endLoopIteration(ExperimentUpLoopScheduler, snapshot)
    );
  }
...
function ExperimentInLoopBegin(ExperimentInLoopScheduler) {
  ...
  for (const thisExperimentIn of ExperimentIn) {
    ...
    ExperimentInLoopScheduler.add(
      endLoopIteration(ExperimentInLoopScheduler, snapshot)
    );
  }
...
function Instructions_5RoutineEachFrame(snapshot) {
  return function () {
    ...
    if (mouse_7.status === PsychoJS.Status.STARTED) {
      // only update if started and not finished!
...
function Instructions_6RoutineEachFrame(snapshot) {
  return function () {
    ...
    if (mouse_8.status === PsychoJS.Status.STARTED) {
      // only update if started and not finished!
...
function Instructions_7RoutineEachFrame(snapshot) {
  return function () {
    ...
    if (mouse_9.status === PsychoJS.Status.STARTED) {
      // only update if started and not finished!
...
function Instructions_8RoutineEachFrame(snapshot) {
  return function () {
    ...
    if (mouse_10.status === PsychoJS.Status.STARTED) {
      // only update if started and not finished!
...
function trial2RoutineEachFrame(snapshot) {
  return function () {
    ...
    if (
      (image_dot.status === PsychoJS.Status.STARTED ||
        image_dot.status === PsychoJS.Status.FINISHED) &&
      t >= frameRemains
    ) {
...
function trialRoutineEnd(snapshot) {
  return function () {
    ...
    // store data for thisExp (ExperimentHandler)
    if (resp.x) {
      psychoJS.experiment.addData("resp.x", resp.x[0]);
    }
    if (resp.y) {
      psychoJS.experiment.addData("resp.y", resp.y[0]);
    }
    if (resp.leftButton) {
      psychoJS.experiment.addData("resp.leftButton", resp.leftButton[0]);
    }
    if (resp.midButton) {
      psychoJS.experiment.addData("resp.midButton", resp.midButton[0]);
    }
    if (resp.rightButton) {
      psychoJS.experiment.addData("resp.rightButton", resp.rightButton[0]);
    }
    if (resp.time) {
      psychoJS.experiment.addData("resp.time", resp.time[0]);
    }
    if (resp.clicked_name) {
      psychoJS.experiment.addData("resp.clicked_name", resp.clicked_name[0]);
    }
...
function Pre_ExperimentRoutineEachFrame(snapshot) {
  return function () {
    ...
    if (mouse_11.status === PsychoJS.Status.STARTED) {
      // only update if started and not finished!
...
function DebreifingRoutineEachFrame(snapshot) {
  return function () {
    ...
    if (mouse_13.status === PsychoJS.Status.STARTED) {
      // only update if started and not finished!
